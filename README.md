...
- **导出**：MP4 视频（H.264，全平台可预览）、WebM 视频（VP9）、PNG 序列（ZIP）；支持导入口播混音（实时录制）。**无声导出采用 WebCodecs 精确编码，帧率严格等于所选值（60fps 就是 60fps）**
...
| 导出 | 顶部工具栏 → 导出动画（MP4 H.264 / WebM VP9 / PNG 序列；无声导出为 WebCodecs 精确帧率编码，导出的视频可直接预览） |
